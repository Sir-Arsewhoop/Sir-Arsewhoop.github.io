...
// process, so keeping every built-site assertion in this one file means Jekyll
// runs once rather than once per test.
...
Expected: PASS, 2 tests. If `bundle install` fails on Windows with a native extension error, the fallback is Docker — but do not spend more than one attempt here; the harness is a convenience and Task 1 can be verified by `bundle exec jekyll build` alone.
...
If the file watcher fails on Windows, add `--force-polling`.

```bash
npm test
```

Runs `node --test test/` — no npm dependencies, nothing to install. `tokens.test.mjs`
checks the palette's contrast ratios and needs no build; `site.test.mjs` builds the
site and asserts against the rendered HTML, so it needs Ruby.
